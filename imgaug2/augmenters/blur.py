...
def blur_gaussian_(
    ...
    **Supported dtypes**:
    ...
    iadt.gate_dtypes_strs(
        {image.dtype},
        allowed="bool uint8 uint16 uint32 int8 int16 int32 int64 uint64 float16 float32 float64",
        disallowed="float128",
        augmenter=None,
    )

    dts_not_supported_by_cv2 = iadt._convert_dtype_strs_to_types("uint32 uint64 int64 float128")
    backend_to_use = backend
    if backend == "auto":
        backend_to_use = "cv2" if image.dtype not in dts_not_supported_by_cv2 else "scipy"
    elif backend == "cv2":
...
def _blur_gaussian_cv2(image: Array, sigma: float, ksize: int | None) -> Array:
    ...
    if ksize is None:
        ...
        assert ia.is_single_integer(ksize), (
            f"Expected 'ksize' argument to be a number, got {type(ksize)}."
        )
...
def blur_mean_shift_(image: Array, spatial_window_radius: int, color_window_radius: int) -> Array:
    ...
    **Supported dtypes**:
    ...
    assert shape_is_hw or shape_is_hw1 or shape_is_hw3, (
        f"Expected (H,W) or (H,W,1) or (H,W,3) image, got shape {image.shape}."
    )
...
class GaussianBlur(meta.Augmenter):
    ...
    **Supported dtypes**:
    ...
    See ``~imgaug2.augmenters.blur.blur_gaussian_(backend="auto")``.
...
class AverageBlur(meta.Augmenter):
    ...
    **Supported dtypes**:
    ...
    Parameters
    ----------
    k : int or tuple of int or tuple of tuple of int or imgaug2.parameters.StochasticParameter or tuple of StochasticParameter, optional
        Kernel size to use.
        ...
        if ia.is_single_number(k):
            ...
            if all([ia.is_single_number(ki) for ki in k]):
                ...
                if ia.is_single_number(k[0]):
                    ...
                    raise Exception(
                        f"k[0] expected to be int or tuple of two ints, got {type(k[0])}"
                    )
                ...
                if ia.is_single_number(k[1]):
                    ...
                    raise Exception(
                        f"k[1] expected to be int or tuple of two ints, got {type(k[1])}"
                    )
            ...
            raise Exception(
                f"Expected int, tuple/list with 2 entries or StochasticParameter. Got {type(k)}."
            )
...
class MedianBlur(meta.Augmenter):
    ...
    **Supported dtypes**:
    ...
    Parameters
    ----------
    k : int or tuple of int or list of int or imgaug2.parameters.StochasticParameter, optional
        Kernel size.
...
class BilateralBlur(meta.Augmenter):
    ...
    **Supported dtypes**:
    ...
    Parameters
    ----------
    d : int or tuple of int or list of int or imgaug2.parameters.StochasticParameter, optional
    ...
    sigma_color : number or tuple of number or list of number or imgaug2.parameters.StochasticParameter, optional
    ...
    sigma_space : number or tuple of number or list of number or imgaug2.parameters.StochasticParameter, optional
...
class MotionBlur(iaa_convolutional.Convolve):
    ...
    **Supported dtypes**:
    ...
    See :class:`~imgaug2.augmenters.convolutional.Convolve`.
    ...
    angle : number or tuple of number or list of number or imgaug2.parameters.StochasticParameter, optional
    ...
    direction : number or tuple of number or list of number or imgaug2.parameters.StochasticParameter, optional
    ...
    def __init__(
        ...
        super().__init__(
            matrix_gen, seed=seed, name=name, random_state=random_state, deterministic=deterministic
        )
...
class MeanShiftBlur(meta.Augmenter):
    ...
    **Supported dtypes**:
    ...
    See :func:`~imgaug2.augmenters.blur.blur_mean_shift_`.

    Parameters
    ----------
    spatial_radius : number or tuple of number or list of number or imgaug2.parameters.StochasticParameter, optional
        Spatial radius for pixels that are assumed to be similar.
    ...
    color_radius : number or tuple of number or list of number or imgaug2.parameters.StochasticParameter, optional
        Color radius for pixels that are assumed to be similar.
        ...
        if batch.images is not None:
            ...
            for i, image in enumerate(batch.images):
                batch.images[i] = blur_mean_shift_(
                    image, spatial_window_radius=samples[0][i], color_window_radius=samples[1][i]
                )
