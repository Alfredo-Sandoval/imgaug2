...
def _assert_exactly_n_shapes(shapes, n, from_ntype, to_ntype):
    if shapes is None:
        raise ValueError(
            ...
            f"This required {n} corresponding image shapes, but argument "
            "'shapes' was set to None. This can happen e.g. if no images were "
            "provided in a Batch, as these would usually be used to "
            "automatically derive image shapes."
        )

    if len(shapes) != n:
        raise ValueError(
            f"Tried to convert data of form '{from_ntype}' to '{to_ntype}'. "
            f"This required exactly {n} corresponding image shapes, but "
            f"instead {len(shapes)} were provided. This can happen e.g. if "
            "more images were provided than corresponding augmentables, "
            "e.g. 10 images but only 5 segmentation maps. It can also happen "
            "if there was a misunderstanding about how an augmentable input "
            "would be parsed. E.g. if a list of N (x,y)-tuples was provided as "
            ...
            "KeypointsOnImage for keypoints instead of lists of tuples."
        )
...
def _assert_single_array_ndim(arr, ndim, shape_str, to_ntype):
    if arr.ndim != ndim:
        raise ValueError(
            ...
            f"that array to be of shape {shape_str}, i.e. {ndim}-dimensional, "
            f"but got {arr.ndim} dimensions instead."
        )
...
def _assert_many_arrays_ndim(arrs, ndim, shape_str, to_ntype):
    ...
    if any([arr.ndim != ndim for arr in arrs_flat]):
        raise ValueError(
            f"Tried to convert an {iterable_type_str} of arrays to a list of "
            f"{to_ntype}. Expected each array to be of shape {shape_str}, "
            f"i.e. to be {ndim}-dimensional, but got dimensions "
            f'{", ".join([str(arr.ndim) for arr in arrs_flat])} '
            f'instead (array shapes: {", ".join([str(arr.shape) for arr in arrs_flat])}).'
        )
...
def _assert_single_array_last_dim_exactly(arr, size, to_ntype):
    if arr.shape[-1] != size:
        raise ValueError(
            ...
            f"the array's last dimension to have size {size}, but got "
            f"{arr.shape[-1]} instead (array shape: {arr.shape})."
        )
...
def _assert_many_arrays_last_dim_exactly(arrs, size, to_ntype):
    ...
    if any([arr.shape[-1] != size for arr in arrs_flat]):
        raise ValueError(
            f"Tried to convert an {iterable_type_str} of array to a list of "
            f"{to_ntype}. Expected the arrays' last dimensions to have size "
            f"{size}, but got "
            f'{", ".join([str(arr.shape[-1]) for arr in arrs_flat])} '
            f'instead (array shapes: {", ".join([str(arr.shape) for arr in arrs_flat])}).'
        )
...
def normalize_images(images):
    ...
    if ia.is_iterable(images):
        ...
        for image in images:
            assert image.ndim in [2, 3], (
                ...
                f"(H,W) or (H,W,C). Got {image.ndim} dimensions instead."
            )
    ...
    raise ValueError(
        ...
        f"None or array or iterable of array. Got type: {type(images)}."
    )
...
def normalize_heatmaps(inputs, shapes=None):
    ...
    _assert_exactly_n_shapes_partial = functools.partial(
        _assert_exactly_n_shapes, from_ntype=ntype, to_ntype="List[HeatmapsOnImage]", shapes=shapes
    )
...
def normalize_keypoints(inputs, shapes=None):
    ...
    _assert_exactly_n_shapes_partial = functools.partial(
        _assert_exactly_n_shapes, from_ntype=ntype, to_ntype="List[KeypointsOnImage]", shapes=shapes
    )
...
def _normalize_polygons_and_line_strings(
    ...
    axis_names_3_str = f"({axis_names[0]},{axis_names[1]},2)"
    axis_names_2_str = f"({axis_names[1]},2)"

    shapes = _preprocess_shapes(shapes)
    ntype = estimate_ntype_func(inputs)
    _assert_exactly_n_shapes_partial = functools.partial(
        _assert_exactly_n_shapes, from_ntype=ntype, to_ntype=(f"List[{cls_oi_name}]"), shapes=shapes
    )
...
def invert_normalize_images(images, images_old):
    if images_old is None:
        assert images is None, (
            ...
            f"'images_old' being None. Got type {type(images)} instead."
        )
        return None
        ...
        for image, image_old in zip(images, images_old):
            if image_old.ndim == 2:
                assert image.shape[2] == 1, (
                    ...
                    f"Got instead C={image.shape[2]} and shape {image.shape}."
                )
                result.append(image[:, :, 0])
            else:
                assert image_old.ndim == 3, (
                    "Expected 'image_old' to be three-dimensional, "
                    f"got {image_old.ndim} dimensions and shape {image_old.shape}."
                )
                result.append(image)
        return result
    raise ValueError(
        "Expected argument 'images_old' to be any of the following: "
        f"None or array or iterable of array. Got type: {type(images_old)}."
    )
...
def invert_normalize_heatmaps(heatmaps, heatmaps_old):
    ...
    if ntype == "None":
        assert heatmaps is None, (
            ...
            f"'heatmaps_old' being None. Got type {type(heatmaps)} instead."
        )
        return heatmaps
    if ntype == "array[float]":
    ...
    if ntype == "iterable[empty]":
        assert heatmaps is None, (
            ...
            f"Got type {type(heatmaps)} instead."
        )
        return []
    if ntype == "iterable-array[float]":
...
def invert_normalize_segmentation_maps(segmentation_maps, segmentation_maps_old):
    ...
    if ntype == "None":
        assert segmentation_maps is None, (
            ...
            f"(unnormalized) 'segmentation_maps_old' being None. Got type {type(segmentation_maps)} "
            "instead."
        )
    ...
    if ntype == "iterable[empty]":
        assert segmentation_maps is None, (
            ...
            f"iterable. Got type {type(segmentation_maps)} instead."
        )
        return []
    if ntype in ["iterable-array[int]", "iterable-array[uint]", "iterable-array[bool]"]:
...
def invert_normalize_keypoints(keypoints, keypoints_old):
    ...
    if ntype == "None":
        assert keypoints is None, (
            ...
            f"'keypoints_old' being None. Got type {type(keypoints)} instead."
        )
        return keypoints
    if ntype in ["array[float]", "array[int]", "array[uint]"]:
    ...
    if ntype == "tuple[number,size=2]":
        ...
        assert len(keypoints[0].keypoints) == 1, (
            ...
            f"before normalization. Got {len(keypoints[0].keypoints)} keypoints instead."
        )
    ...
    if ntype == "iterable[empty]":
        assert keypoints is None, (
            ...
            f"iterable. Got type {type(keypoints)} instead."
        )
        return []
    if ntype in ["iterable-array[float]", "iterable-array[int]", "iterable-array[uint]"]:
    ...
    if ntype == "iterable-tuple[number,size=2]":
        assert len(keypoints) == 1, (
            ...
            f"normalization. Got {len(keypoints)} instances instead."
        )
...
def invert_normalize_bounding_boxes(bounding_boxes, bounding_boxes_old):
    ...
    if ntype == "None":
        assert bounding_boxes is None, (
            ...
            f"(unnormalized) 'bounding_boxes_old' being None. Got type {type(bounding_boxes)} "
            "instead."
        )
    ...
    if ntype == "tuple[number,size=4]":
        ...
        assert len(bounding_boxes[0].bounding_boxes) == 1, (
            ...
            "(x1,y1,x2,y2) tuple before normalization. "
            f"Got {len(bounding_boxes[0].bounding_boxes)} bounding boxes instead."
        )
    ...
    if ntype == "BoundingBox":
        ...
        assert len(bounding_boxes[0].bounding_boxes) == 1, (
            "Expected a BoundingBoxesOnImage instance containing a single "
            "BoundingBox after normalization due to getting a single "
            "BoundingBox before normalization. "
            f"Got {len(bounding_boxes[0].bounding_boxes)} bounding boxes instead."
        )
    ...
    if ntype == "BoundingBoxesOnImage":
        assert len(bounding_boxes) == 1, (
            ...
            f"before normalization. Got {len(bounding_boxes)} instances instead."
        )
    ...
    if ntype == "iterable[empty]":
        assert bounding_boxes is None, (
            ...
            f"iterable. Got type {type(bounding_boxes)} instead."
        )
        return []
    if ntype in ["iterable-array[float]", "iterable-array[int]", "iterable-array[uint]"]:
    ...
    if ntype == "iterable-tuple[number,size=4]":
        assert len(bounding_boxes) == 1, (
            ...
            f"tuples before normalization. Got {len(bounding_boxes)} instances instead."
        )
...
def invert_normalize_line_strings(line_strings, line_strings_old):
    return _invert_normalize_polygons_and_line_strings(
        line_strings,
        line_strings_old,
        estimate_line_strings_norm_type,
        "LineString",
        "LineStringsOnImage",
...
def _invert_normalize_polygons_and_line_strings(
    ...
    if ntype == "None":
        assert inputs is None, (
            ...
            f"(unnormalized) polygons/line strings being None. Got type {type(inputs)} "
            "instead."
        )
        return inputs
    if ntype in ["array[float]", "array[int]", "array[uint]"]:
    ...
    if ntype == cls_single_name:
        assert len(inputs) == 1, (
            ...
            f"Got {len(inputs)} instances instead."
        )
        assert len(get_entities_func(inputs[0])) == 1, (
            f"Expected a {cls_oi_name} instance containing a single "
            f"{cls_single_name} after normalization due to getting a single {cls_single_name} "
            f"before normalization. Got {len(get_entities_func(inputs[0]))} instances instead."
        )
    ...
    if ntype == "iterable[empty]":
        assert inputs is None, (
            ...
            f"empty iterable. Got type {type(inputs)} instead."
        )
        return []
    if ntype in ["iterable-array[float]", "iterable-array[int]", "iterable-array[uint]"]:
    ...
    if ntype == "iterable-tuple[number,size=2]":
        ...
        assert len(get_entities_func(inputs[0])) == 1, (
            f"Expected a {cls_oi_name} instance after normalization "
            f"containing a single {cls_single_name} instance due to getting an iterable "
    ...
    if ntype == (f"iterable-{cls_single_name}"):
        ...
        assert len(get_entities_func(inputs[0])) == len(inputs_old), (
            f"Expected a {cls_oi_name} instance after normalization "
            f"containing a single {cls_single_name} instance due to getting an iterable "
            f"of {cls_single_name} before normalization. "
            f"Got a {cls_oi_name} with {len(inputs)} {cls_single_name} instances instead."
        )
...
def estimate_segmaps_norm_type(segmentation_maps):
    ...
    _assert_is_of_norm_type(type_str, valid_type_strs, "segmentation_maps")
    return type_str
...
def estimate_bounding_boxes_norm_type(bounding_boxes):
    ...
    _assert_is_of_norm_type(type_str, valid_type_strs, "bounding_boxes")
    return type_str
...
def _estimate_polygons_and_line_segments_norm_type(
    ...
    valid_type_strs = [
        ...
        "iterable-tuple[number,size=2]",
        "iterable-Keypoint",
        f"iterable-{cls_single_name}",
        f"iterable-{cls_oi_name}",
        "iterable-iterable[empty]",
        "iterable-iterable-array[float]",
        "iterable-iterable-array[int]",
        "iterable-iterable-array[uint]",
        "iterable-iterable-tuple[number,size=2]",
        "iterable-iterable-Keypoint",
        f"iterable-iterable-{cls_single_name}",
...
def find_first_nonempty(attr, parents=None):
    ...
    if _is_iterable(attr):
        ...
        for attr_i in attr:
            nonempty, success, parents_found = find_first_nonempty(attr_i, parents=parents + [attr])
            if success:
...
def _nonempty_info_to_type_str(nonempty, success, parents):
    ...
    parent_iters = ""
    if len(parents) > 0:
        parent_iters = f'{"-".join(["iterable"] * len(parents))}-'

    if not success:
        return f"{parent_iters}iterable[empty]"

    is_parent_tuple = len(parents) >= 1 and isinstance(parents[-1], tuple)

    if is_parent_tuple:
        is_only_numbers_in_tuple = len(parents[-1]) > 0 and all(
            [ia.is_single_number(val) for val in parents[-1]]
        )

        if is_only_numbers_in_tuple:
            parent_iters = "-".join(["iterable"] * (len(parents) - 1))
            tpl_name = f"tuple[number,size={len(parents[-1])}]"
    ...
    assert isinstance(nonempty, object), (
        f"Expected 'nonempty' to be an object, got type {type(nonempty)}."
    )
    return f"{parent_iters}{nonempty.__class__.__name__}"
