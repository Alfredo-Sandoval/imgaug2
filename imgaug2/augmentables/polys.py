...
def recover_psois_(
    ...
    Parameters
    ----------
    psois : list of imgaug2.augmentables.polys.PolygonsOnImage or imgaug2.augmentables.polys.PolygonsOnImage
        The possibly broken polygons, e.g. after augmentation.
        The `recoverer` is applied to them.

    psois_orig : list of imgaug2.augmentables.polys.PolygonsOnImage or imgaug2.augmentables.polys.PolygonsOnImage
    ...
    for i, psoi in enumerate(psois):
        for j, polygon in enumerate(psoi.polygons):
            poly_rec = recoverer.recover_from(
                polygon.exterior, psois_orig[i].polygons[j], random_state
            )
...
class Polygon:
    ...
    Parameters
    ----------
    exterior : list of imgaug2.augmentables.kps.Keypoint or list of tuple of float or (N,2) ndarray
        List of points defining the polygon. May be either a ``list`` of
        :class:`~imgaug2.augmentables.kps.Keypoint` objects or a ``list`` of
    ...
    def __init__(
        ...
        # automatically.
        is_closed = len(self.exterior) >= 2 and np.allclose(
            self.exterior[0, :], self.exterior[-1, :]
        )
        if is_closed:
    ...
    def is_out_of_image(self, image: ShapeLike, fully: bool = True, partly: bool = False) -> bool:
        ...
        if len(self.exterior) == 0:
            raise Exception(
                "Cannot determine whether the polygon is inside "
    ...
    def clip_out_of_image(self, image: ShapeLike) -> list[Polygon]:
        ...
        Returns
        -------
        list of imgaug2.augmentables.polys.Polygon
        ...
        elif isinstance(multipoly_inter_shapely, shapely.geometry.GeometryCollection):
    ...
    def change_first_point_by_index(self, point_idx: int) -> Polygon:
        ...
        exterior = np.concatenate(
            (self.exterior[point_idx:, :], self.exterior[:point_idx, :]), axis=0
        )
    ...
    def to_keypoints(self) -> list[Keypoint]:
        ...
        list of imgaug2.augmentables.kps.Keypoint
            Exterior vertices as :class:`~imgaug2.augmentables.kps.Keypoint`
            instances.
    ...
    def coords_almost_equals(
        ...
        Parameters
        ----------
        other : imgaug2.augmentables.polys.Polygon or (N,2) ndarray or list of tuple
            See
            :func:`~imgaug2.augmentables.polys.Polygon.exterior_almost_equals`.
    ...
    def almost_equals(self, other: Polygon, max_distance: float = 1e-4, points_per_edge: int = 8) -> bool:
        ...
        This is the same as
        :func:`~imgaug2.augmentables.polys.Polygon.exterior_almost_equals` but
        additionally compares the labels.

        Parameters
        ----------
        other : imgaug2.augmentables.polys.Polygon
    ...
    def copy(self, exterior: NDArray[np.number] | list[Point2D] | list[Keypoint] | None = None, label: str | None = None) -> Polygon:
        ...
        Parameters
        ----------
        exterior : list of imgaug2.augmentables.kps.Keypoint or list of tuple or (N,2) ndarray, optional
            List of points defining the polygon. See
            :func:`~imgaug2.augmentables.polys.Polygon.__init__` for details.
    ...
    def deepcopy(
        ...
        exterior : list of Keypoint or list of tuple or (N,2) ndarray, optional
            List of points defining the polygon. See
            `imgaug2.augmentables.polys.Polygon.__init__` for details.
...
class PolygonsOnImage(IAugmentable):
    ...
    Parameters
    ----------
    polygons : list of imgaug2.augmentables.polys.Polygon
    ...
    def copy(self, polygons: list[Polygon] | None = None, shape: Shape | None = None) -> PolygonsOnImage:
        ...
        Parameters
        ----------
        polygons : None or list of imgaug2.augmentables.polys.Polygons, optional
            List of polygons on the image.
...
class _ConcavePolygonRecoverer:
    ...
    def _generate_intersection_points(
        ...
        segments = [
            (
                ...
                (
                    np.round(float(exterior[(i + 1) % len(exterior)][0]), decimals),
                    np.round(float(exterior[(i + 1) % len(exterior)][1]), decimals),
                ),
            )
    ...
    def _fit_best_valid_polygon(
        ...
        while not converged:
            ...
            combos = np.transpose(
                [
                    np.tile(np.int32(points_left), len(np.int32(points_kept))),
                    np.repeat(np.int32(points_kept), len(np.int32(points_left))),
                ]
            )
            combos = np.concatenate(
                (combos, np.zeros((combos.shape[0], 3), dtype=np.int32)), axis=1
            )
            ...
            # <point_left>
            combos[:, 4] = np.minimum(
                combos[:, 3] - combos[:, 2], len(points) - combos[:, 3] + combos[:, 2]
            )
            ...
            candidate_ids = sorted(
                candidate_ids, key=lambda idx: (candidates[idx][2], candidates[idx][3])
            )
            ...
            has_reached_iters_max = (
                self.fit_n_iters_max is not None and iteration > self.fit_n_iters_max
            )
...
class MultiPolygon:
    ...
    Parameters
    ----------
    geoms : list of imgaug2.augmentables.polys.Polygon
    ...
    def from_shapely(geometry: object, label: str | None = None) -> MultiPolygon:
        ...
        Returns
        -------
        imgaug2.augmentables.polys.MultiPolygon
