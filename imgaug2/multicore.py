...
class Pool:
    ...
    Parameters
    ----------
    augseq : imgaug2.augmenters.meta.Augmenter
    ...
    def __init__(
        ...
        assert processes is None or processes != 0, (
            ...
            f"processes\"). Got type {type(processes)}, value {str(processes)} instead."
        )
        ...
        if seed is not None:
            assert iarandom.SEED_MIN_VALUE <= seed <= iarandom.SEED_MAX_VALUE, (
                ...
                f"{iarandom.SEED_MIN_VALUE} and {iarandom.SEED_MAX_VALUE}. "
                f"Got type {type(seed)}, value {str(seed)} instead."
            )
        self.seed = seed
    ...
    def map_batches(
        self, batches: list[Batch], chunksize: int | None = None
    ) -> list[Batch]:
        """
        ...
        Parameters
        ----------
        batches : list of imgaug2.augmentables.batches.Batch
        ...
        Returns
        -------
        list of imgaug2.augmentables.batches.Batch
            Augmented batches.
    ...
    @classmethod
    def _assert_batches_is_list(cls, batches: Any) -> None:  # noqa: ANN401
        assert isinstance(batches, list), (
            f"Expected `batches` to be a list, got type {type(batches)}. Call "
    ...
    def imap_batches(
        ...
        Parameters
        ----------
        batches : generator of imgaug2.augmentables.batches.Batch
    ...
    @classmethod
    def _assert_batches_is_generator(cls, batches: Any) -> None:  # noqa: ANN401
        assert ia.is_generator(batches), (
            f"Expected `batches` to be generator, got type {type(batches)}. Call "
    ...
    def join(self) -> None:
        ...
        :func:`~imgaug2.multicore.Pool.close` or
        :func:`~imgaug2.multicore.Pool.terminate`.

        """
        if self._pool is not None:
            self._pool.join()

    def _handle_batch_ids(self, batches: list[Batch]) -> list[tuple[int, Batch]]:
        ids = np.arange(self._batch_idx, self._batch_idx + len(batches))
        inputs = list(zip(ids, batches, strict=True))
        self._batch_idx += len(batches)
        return inputs

    def _handle_batch_ids_gen(
        self, batches: Generator[Batch, None, None]
    ) -> Generator[tuple[int, Batch], None, None]:
    ...
    @classmethod
    def _ibuffer_batch_loading(
        cls, batches: Generator[Batch, None, None], output_buffer_left: Any  # noqa: ANN401
    ) -> Generator[Batch, None, None]:
        for batch in batches:
            if output_buffer_left is not None:
...
def _create_output_buffer_left(output_buffer_size: int | None) -> Any:  # noqa: ANN401
    output_buffer_left = None
    if output_buffer_size:
        assert output_buffer_size > 0, (
            f"Expected buffer size to be greater than zero, but got size {output_buffer_size} "
            "instead."
        )
...
def _Pool_worker(batch_idx: int, batch: Batch | UnnormalizedBatch) -> Batch | UnnormalizedBatch:
    ...
    assert isinstance(batch, (UnnormalizedBatch, Batch)), (
        ...
        "`imgaug2.augmentables.batches.UnnormalizedBatch` or "
        f"`imgaug2.augmentables.batches.Batch`. Got type {type(batch)} instead."
    )
    assert Pool._WORKER_AUGSEQ is not None, (
        "Expected `Pool._WORKER_AUGSEQ` to NOT be `None`. Did you manually call _Pool_worker()?"
    )
...
def _reseed_global_local(base_seed: int, augseq: Augmenter) -> None:
...
def _derive_seed(base_seed: int, offset: int = 0) -> int:
...
class BatchLoader:
    ...
    Loaded batches can be accesses using :attr:`imgaug2.BatchLoader.queue`.
    ...
    @ia.deprecated(alt_func="imgaug2.multicore.Pool")
    def __init__(
        self,
        load_batch_func: Callable[[], Batch | list[Batch]] | Generator[Batch, None, None],
        queue_size: int = 50,
        nb_workers: int = 1,
        threaded: bool = True,
    ) -> None:
        assert queue_size >= 2, f"Queue size for BatchLoader must be at least 2, got {queue_size}."
        assert nb_workers >= 1, (
    ...
    def count_workers_alive(self) -> int:
    ...
    def _load_batches(
        cls,
        load_batch_func: Callable[[], Batch | list[Batch]] | Generator[Batch, None, None],
        queue_internal: Any,  # noqa: ANN401
        ...
        try:
            ...
            for batch in gen:
                assert isinstance(batch, Batch), (
                    ...
                    f"be of class imgaug2.Batch, got {type(batch)}."
                )
...
class BackgroundAugmenter:
    ...
    **Deprecated**. Augment batches in the background processes.
    ...
    def __init__(
        self,
        batch_loader: BatchLoader | Any,  # noqa: ANN401
        augseq: Augmenter,
        queue_size: int = 50,
        nb_workers: str | int = "auto",
    ) -> None:
        assert queue_size > 0, f"Expected 'queue_size' to be at least 1, got {queue_size}."
        self.augseq = augseq
        self.queue_source = (
            batch_loader
        ...
        if nb_workers == "auto":
            ...
            assert nb_workers >= 1, (
                f"Expected 'nb_workers' to be \"auto\" or at least 1, got {nb_workers} instead."
            )
    ...
    def get_batch(self) -> Batch | None:
        ...
        # SECURITY: This unpickles data originating from our own worker
        # processes via a local multiprocessing queue. Do not feed untrusted
        # serialized input into the pool/queues.
        batch = pickle.loads(batch_str)  # nosec B301
    ...
    @classmethod
    def _augment_images_worker(
        cls,
        augseq: Augmenter,
        queue_source: Any,  # noqa: ANN401
        queue_result: Any,  # noqa: ANN401
        seedval: int,
    ) -> None:
        """
        ...
        while not loader_finished:
            ...
            try:
                ...
                # SECURITY: See comment in Pool.get_batch().
                batch = pickle.loads(batch_str)  # nosec B301
                if batch is None:
