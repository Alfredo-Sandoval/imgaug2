...
def runtest_pickleable_uint8_img(augmenter, shape=(15, 15, 3), iterations=3):
    ...
    # should only ever unpickle data that this function pickled itself.
    augmenter_pkl = pickle.loads(pickle.dumps(augmenter, protocol=-1))  # nosec B301
...
def is_parameter_instance(param, param_type):
    ...
    is an instance of :class:`imgaug2.parameters.AutoPrefetcher`, then it
    ...
    Parameters
    ----------
    param : imgaug2.parameters.StochasticParameter
        The parameter to check.

    param_type : type
        The desired type. Similar as in ``isinstance``.
        E.g. ``imgaug2.parameters.Deterministic``.
