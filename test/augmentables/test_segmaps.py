...
class TestSegmentationMapsOnImage___init__(unittest.TestCase):
    def test_uint_int_arrs(self):
        ...
        for dtype, ndim, img_shape in gen:
            with self.subTest(dtype=dtype, ndim=ndim, shape=img_shape):
                ...
                arr = np.array([[0, 0, 1], [0, 2, 1], [1, 3, 1]], dtype=dtype).reshape(shape)
                segmap = ia.SegmentationMapsOnImage(arr, shape=img_shape)
                assert segmap.shape == img_shape
                assert segmap.arr.dtype.name == "int32"
                assert segmap.arr.shape == (3, 3, 1)
                assert np.array_equal(segmap.arr, arr.reshape((3, 3, 1)).astype(np.int32))

                if ndim == 3:
                    arr = np.array([[0, 0, 1], [0, 2, 1], [1, 3, 1]], dtype=dtype).reshape(
                        (3, 3, 1)
                    )
    ...
    def test_bool_arr_2d(self):
        arr = np.array([[0, 0, 1], [0, 1, 1], [1, 1, 1]], dtype=bool).reshape((3, 3))
    ...
    def test_bool_arr_3d(self):
        arr = np.array([[0, 0, 1], [0, 1, 1], [1, 1, 1]], dtype=bool).reshape((3, 3, 1))
        arr = np.tile(arr, (1, 1, 5))
    ...
    def test_boolean_masks(self):
        ...
        for dt in [bool, np.bool_]:
            ...
            assert np.array_equal(
                segmap.arr, np.int32([[0, 0, 0], [0, 1, 0], [0, 0, 0]])[:, :, np.newaxis]
            )
    ...
    def test_uint32_fails(self):
        got_exception = False
        try:
            arr = np.array([[0, 0, 1], [0, 2, 1], [1, 3, 1]], dtype=np.uint32)
    ...
    def test_legacy_support_for_float32_2d(self):
        ...
        assert len(caught_warnings) == 1
        assert "Got a float array as the segmentation map in" in str(caught_warnings[0].message)

    def test_legacy_support_for_float32_3d(self):
        arr = np.array([[[0.4, 0.6], [0.2, 0.1]]], dtype=np.float32).reshape((1, 2, 2))
        with warnings.catch_warnings(record=True) as caught_warnings:
            warnings.simplefilter("always")
            segmap = segmapslib.SegmentationMapsOnImage(arr, shape=(1, 1, 3))

            arr_expected = np.array([[1, 0]], dtype=np.int32).reshape((1, 2, 1))
            assert np.array_equal(segmap.arr, arr_expected)
            assert segmap.shape == (1, 1, 3)
    ...
    def test_bool(self):
        ...
        for ndim in ndims:
            with self.subTest(ndim=ndim):
                shape = (3, 3) if ndim == 2 else (3, 3, 1)
                arr = np.array([[0, 0, 1], [0, 1, 1], [1, 1, 1]], dtype=bool).reshape(shape)
                segmap = ia.SegmentationMapsOnImage(arr, shape=(3, 3))
...
class TestSegmentationMapsOnImage_draw(unittest.TestCase):
    ...
    def test_with_two_classes(self):
        ...
        col0 = self.col(0)
        col1 = self.col(1)
        expected = np.uint8([[col0, col1, col1], [col0, col1, col1], [col0, col1, col1]])
    ...
    def test_use_size_arg_to_resize_to_2x(self):
        ...
        double_size_args = [(6, 6), (2.0, 2.0), 6, 2.0]
    ...
    def test_use_size_arg_to_keep_at_same_size(self):
        ...
        size_args = [None, (None, None), (3, None), (None, 3), (1.0, None), (None, 1.0)]

        col0 = self.col(0)
        col1 = self.col(1)
        expected = np.uint8([[col0, col1, col1], [col0, col1, col1], [col0, col1, col1]])
        expected = ia.imresize_single_image(expected, (3, 3), interpolation="nearest")
    ...
    def test_segmap_with_more_than_one_channel(self):
        ...
        arr_multi = np.stack([arr_channel_1, arr_channel_2, arr_channel_3], axis=-1)

        col = ia.SegmentationMapsOnImage.DEFAULT_SEGMENT_COLORS
        expected_channel_1 = np.uint8(
            [[col[0], col[1], col[5]], [col[0], col[1], col[1]], [col[0], col[4], col[1]]]
        )
        expected_channel_2 = np.uint8(
            [[col[1], col[1], col[0]], [col[2], col[2], col[0]], [col[1], col[1], col[0]]]
        )
        expected_channel_3 = np.uint8(
            [[col[1], col[0], col[0]], [col[0], col[1], col[0]], [col[0], col[0], col[3]]]
        )
...
class TestSegmentationMapsOnImage_draw_on_image(unittest.TestCase):
    ...
    def test_alpha_only_segmap_is_visible(self):
        # only segmap visible
        observed = self.segmap.draw_on_image(self.image, alpha=1.0, draw_background=True)
        col0 = self.col(0)
        col1 = self.col(1)
        expected = np.uint8([[col0, col1, col1], [col0, col1, col1], [col0, col1, col1]])
        assert isinstance(observed, list)
    ...
    def test_alpha_with_draw_background(self):
        ...
        observed = self.segmap.draw_on_image(image, alpha=1.0, draw_background=False)
    ...
    def test_non_binary_alpha_with_draw_background_and_bg_class_id(self):
        ...
        observed = segmap.draw_on_image(
            image, alpha=a1, draw_background=False, background_class_id=1
        )
    ...
    def test_resize_segmentation_map_to_image(self):
        ...
        observed = segmap.draw_on_image(
            image, alpha=a1, draw_background=True, resize="segmentation_map"
        )
    ...
    def test_background_threshold_leads_to_deprecation_warning(self):
        ...
        assert len(caught_warnings) == 1
        assert "The argument `background_threshold` is deprecated" in str(
...
class TestSegmentationMapsOnImage_pad(unittest.TestCase):
    @property
    def segmap(self):
        arr = np.int32([[0, 1, 1], [0, 2, 1], [0, 1, 3]])
        return ia.SegmentationMapsOnImage(arr, shape=(3, 3))
    ...
    def test_default_pad_mode_and_cval(self):
        ...
        expected = np.pad(
            self.segmap.arr, ((1, 3), (4, 2), (0, 0)), mode="constant", constant_values=0
        )
        assert np.array_equal(observed, expected)

    def test_default_pad_mode(self):
        segmap_padded = self.segmap.pad(top=1, right=2, bottom=3, left=4, cval=1.0)
        observed = segmap_padded.arr

        expected = np.pad(
            self.segmap.arr, ((1, 3), (4, 2), (0, 0)), mode="constant", constant_values=1.0
        )
        assert np.array_equal(observed, expected)

    def test_default_cval(self):
        segmap_padded = self.segmap.pad(top=1, right=2, bottom=3, left=4, mode="edge")
        observed = segmap_padded.arr

        expected = np.pad(self.segmap.arr, ((1, 3), (4, 2), (0, 0)), mode="edge")
        assert np.array_equal(observed, expected)
...
class TestSegmentationMapsOnImage_pad_to_aspect_ratio(unittest.TestCase):
    ...
    def test_square_ratio_with_default_pad_mode_and_cval(self):
        ...
        expected = np.pad(
            self.segmap.arr, ((0, 1), (0, 0), (0, 0)), mode="constant", constant_values=0
        )
        assert np.array_equal(observed, expected)
    ...
    def test_square_ratio_with_cval_set(self):
        ...
        expected = np.pad(
            self.segmap.arr, ((0, 1), (0, 0), (0, 0)), mode="constant", constant_values=1.0
        )
        assert np.array_equal(observed, expected)
    ...
    def test_square_ratio_with_pad_mode_edge(self):
        ...
        expected = np.pad(self.segmap.arr, ((0, 1), (0, 0), (0, 0)), mode="edge")
        assert np.array_equal(observed, expected)
    ...
    def test_higher_than_wide_ratio_with_default_pad_mode_and_cval(self):
        ...
        expected = np.pad(
            self.segmap.arr, ((2, 2), (0, 0), (0, 0)), mode="constant", constant_values=0
        )
        assert np.array_equal(observed, expected)

    def test_return_pad_amounts(self):
        segmap_padded, pad_amounts = self.segmap.pad_to_aspect_ratio(0.5, return_pad_amounts=True)
        observed = segmap_padded.arr
...
class TestSegmentationMapsOnImage_copy(unittest.TestCase):
    @property
    def segmap(self):
        arr = np.int32([[0, 1], [2, 3]]).reshape((2, 2, 1))
        return ia.SegmentationMapsOnImage(arr, shape=(2, 2))
