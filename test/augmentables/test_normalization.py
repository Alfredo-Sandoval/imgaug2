...
class TestNormalization(unittest.TestCase):
    ...
    def test_invert_normalize_images(self):
        ...
        observed = normalization.invert_normalize_images([arr1, arr2], [arr1_old, arr2_old])
        assert isinstance(observed, list)
    ...
    def test_invert_normalize_heatmaps(self):
        ...
        # ----
        before = ia.HeatmapsOnImage(np.zeros((1, 1, 1), dtype=np.float32) + 0.1, shape=(1, 1, 3))
    ...
    def test_invert_normalize_segmentation_maps(self):
        def _norm_and_invert(segmaps, images):
            return normalization.invert_normalize_segmentation_maps(
                normalization.normalize_segmentation_maps(segmaps, shapes=images), segmaps
            )
        ...
        # ----
        before = [
            ia.SegmentationMapsOnImage(np.zeros((1, 1, 1), dtype=np.int32) + 1, shape=(1, 1, 3))
        ]
        after = _norm_and_invert(before, images=None)
        assert isinstance(after, list)
    ...
    def test_invert_normalize_keypoints(self):
        ...
        after = _norm_and_invert(
            before,
            images=[np.zeros((1, 1, 3), dtype=np.uint8), np.zeros((1, 1, 3), dtype=np.uint8)],
        )
        assert isinstance(after, list)
        assert len(after) == 2
    ...
    def test_invert_normalize_bounding_boxes(self):
        ...
        # ----
        before = ia.BoundingBoxesOnImage([ia.BoundingBox(x1=1, y1=2, x2=3, y2=4)], shape=(1, 1, 3))
        after = _norm_and_invert(before, images=None)
        ...
        # ----
        before = [ia.BoundingBox(x1=1, y1=2, x2=3, y2=4), ia.BoundingBox(x1=5, y1=6, x2=7, y2=8)]
        after = _norm_and_invert(before, images=[np.zeros((1, 1, 3), dtype=np.uint8)])
        assert isinstance(after, list)
        assert len(after) == 2
        ...
        # ----
        before = [
            ia.BoundingBoxesOnImage([ia.BoundingBox(x1=1, y1=2, x2=3, y2=4)], shape=(1, 1, 3)),
            ia.BoundingBoxesOnImage([ia.BoundingBox(x1=5, y1=6, x2=7, y2=8)], shape=(1, 1, 3)),
        ]
        after = _norm_and_invert(before, images=None)
        assert isinstance(after, list)
        assert len(after) == 2
        ...
        # ----
        before = [
            [ia.BoundingBox(x1=1, y1=2, x2=3, y2=4), ia.BoundingBox(x1=5, y1=6, x2=7, y2=8)],
            [ia.BoundingBox(x1=9, y1=10, x2=11, y2=12), ia.BoundingBox(x1=13, y1=14, x2=15, y2=16)],
        ]
        after = _norm_and_invert(
            before,
            images=[np.zeros((1, 1, 3), dtype=np.uint8), np.zeros((1, 1, 3), dtype=np.uint8)],
    ...
    def test_invert_normalize_polygons(self):
        ...
        for dt in [np.dtype("float32"), np.dtype("int16"), np.dtype("uint16")]:
            for images in [
                ...
                before = np.tile(coords1_arr[np.newaxis, np.newaxis, ...].astype(dt), (1, 5, 1, 1))
                after = _norm_and_invert(before, images=images)
                assert ia.is_np_array(after)
    ...
    def test_invert_normalize_line_strings(self):
        def _norm_and_invert(line_strings, images):
            return normalization.invert_normalize_line_strings(
                normalization.normalize_line_strings(line_strings, shapes=images), line_strings
            )
    ...
    def test_normalize_heatmaps(self):
        ...
        # ----
        heatmaps_norm = normalization.normalize_heatmaps(
            ia.HeatmapsOnImage(np.zeros((1, 1, 1), dtype=np.float32) + 0.1, shape=(1, 1, 3)),
            shapes=None,
        )
        assert isinstance(heatmaps_norm, list)
        assert isinstance(heatmaps_norm[0], ia.HeatmapsOnImage)
    ...
    def test_normalize_segmentation_maps(self):
        ...
        for dt in [np.dtype("int32"), np.dtype("uint16"), np.dtype(bool)]:
            ...
            segmaps_norm = normalization.normalize_segmentation_maps(
                np.full((1, 1, 1, 1), 1, dtype=dt), shapes=[np.zeros((1, 1, 3), dtype=np.uint8)]
            )
            ...
            segmaps_norm = normalization.normalize_segmentation_maps(
                np.full((1, 1, 1, 1), 1, dtype=dt), shapes=np.zeros((1, 1, 1, 3), dtype=np.uint8)
            )
            assert isinstance(segmaps_norm, list)
            assert isinstance(segmaps_norm[0], ia.SegmentationMapsOnImage)
            assert np.allclose(segmaps_norm[0].arr[..., 0], 1)
            ...
            with self.assertRaises(ValueError):
                _segmaps_norm = normalization.normalize_segmentation_maps(
                    np.full((2, 1, 1), 1, dtype=dt), shapes=[np.zeros((1, 1, 3), dtype=np.uint8)]
                )

            # --> too few segmaps
            with self.assertRaises(ValueError):
                _segmaps_norm = normalization.normalize_segmentation_maps(
                    np.full((1, 1, 1), 1, dtype=dt), shapes=np.zeros((2, 1, 1, 3), dtype=np.uint8)
                )
        ...
        # ----
        segmaps_norm = normalization.normalize_segmentation_maps(
            ia.SegmentationMapsOnImage(np.full((1, 1, 1), 1, dtype=np.int32), shape=(1, 1, 3)),
            shapes=None,
        )
        assert isinstance(segmaps_norm, list)
        assert isinstance(segmaps_norm[0], ia.SegmentationMapsOnImage)
        ...
        for dt in [np.dtype("int32"), np.dtype("uint16"), np.dtype(bool)]:
            segmaps_norm = normalization.normalize_segmentation_maps(
                [np.full((1, 1, 1), 1, dtype=dt)], shapes=[np.zeros((1, 1, 3), dtype=np.uint8)]
            )
            assert isinstance(segmaps_norm, list)
            assert isinstance(segmaps_norm[0], ia.SegmentationMapsOnImage)
            assert np.allclose(segmaps_norm[0].arr[..., 0], 1)
            ...
            segmaps_norm = normalization.normalize_segmentation_maps(
                [np.full((1, 1, 1), 1, dtype=dt)], shapes=np.zeros((1, 1, 1, 3), dtype=np.uint8)
            )
            assert isinstance(segmaps_norm, list)
            assert isinstance(segmaps_norm[0], ia.SegmentationMapsOnImage)
            assert np.allclose(segmaps_norm[0].arr[..., 0], 1)
            ...
            with self.assertRaises(ValueError):
                _segmaps_norm = normalization.normalize_segmentation_maps(
                    [np.full((1, 1, 1), 1, dtype=np.int32), np.full((1, 1, 1), 1, dtype=np.int32)],
                    shapes=[np.zeros((1, 1, 3), dtype=np.uint8)],
                )
    ...
    def test_normalize_keypoints(self):
        ...
        for dt in [np.dtype("float32"), np.dtype("int16"), np.dtype("uint16")]:
            keypoints_norm = normalization.normalize_keypoints(
                np.zeros((1, 1, 2), dtype=dt) + 1, shapes=[np.zeros((1, 1, 3), dtype=np.uint8)]
            )
            ...
            keypoints_norm = normalization.normalize_keypoints(
                np.zeros((1, 5, 2), dtype=dt) + 1, shapes=np.zeros((1, 1, 1, 3), dtype=np.uint8)
            )
            assert isinstance(keypoints_norm, list)
            assert isinstance(keypoints_norm[0], ia.KeypointsOnImage)
            ...
            with self.assertRaises(ValueError):
                _keypoints_norm = normalization.normalize_keypoints(
                    np.zeros((2, 1, 2), dtype=dt) + 1, shapes=[np.zeros((1, 1, 3), dtype=np.uint8)]
                )

            # --> too few keypoints
            with self.assertRaises(ValueError):
                _keypoints_norm = normalization.normalize_keypoints(
                    np.zeros((1, 1, 2), dtype=dt) + 1, shapes=np.zeros((2, 1, 1, 3), dtype=np.uint8)
                )
        ...
        for dt in [np.dtype("float32"), np.dtype("int16"), np.dtype("uint16")]:
            keypoints_norm = normalization.normalize_keypoints(
                [np.zeros((1, 2), dtype=dt) + 1], shapes=[np.zeros((1, 1, 3), dtype=np.uint8)]
            )
            assert isinstance(keypoints_norm, list)
            assert isinstance(keypoints_norm[0], ia.KeypointsOnImage)
            assert len(keypoints_norm[0].keypoints) == 1
            ...
            keypoints_norm = normalization.normalize_keypoints(
                [np.zeros((5, 2), dtype=dt) + 1], shapes=np.zeros((1, 1, 1, 3), dtype=np.uint8)
            )
            assert isinstance(keypoints_norm, list)
            assert isinstance(keypoints_norm[0], ia.KeypointsOnImage)
            assert len(keypoints_norm[0].keypoints) == 5
            ...
            with self.assertRaises(ValueError):
                _keypoints_norm = normalization.normalize_keypoints(
                    [np.zeros((1, 2), dtype=dt) + 1], shapes=np.zeros((2, 1, 1, 3), dtype=np.uint8)
                )
        ...
        with self.assertRaises(ValueError):
            _keypoints_norm = normalization.normalize_keypoints(
                [(1, 2)],
                shapes=[np.zeros((1, 1, 3), dtype=np.uint8), np.zeros((1, 1, 3), dtype=np.uint8)],
            )
    ...
    def test_normalize_bounding_boxes(self):
        ...
        for dt in [np.dtype("float32"), np.dtype("int16"), np.dtype("uint16")]:
            bbs_norm = normalization.normalize_bounding_boxes(
                np.zeros((1, 1, 4), dtype=dt) + 1, shapes=[np.zeros((1, 1, 3), dtype=np.uint8)]
            )
            ...
            bbs_norm = normalization.normalize_bounding_boxes(
                np.zeros((1, 5, 4), dtype=dt) + 1, shapes=np.zeros((1, 1, 1, 3), dtype=np.uint8)
            )
            assert isinstance(bbs_norm, list)
            assert isinstance(bbs_norm[0], ia.BoundingBoxesOnImage)
            ...
            with self.assertRaises(ValueError):
                _bbs_norm = normalization.normalize_bounding_boxes(
                    np.zeros((2, 1, 4), dtype=dt) + 1, shapes=[np.zeros((1, 1, 3), dtype=np.uint8)]
                )

            # --> too few bounding boxes
            with self.assertRaises(ValueError):
                _bbs_norm = normalization.normalize_bounding_boxes(
                    np.zeros((1, 1, 4), dtype=dt) + 1, shapes=np.zeros((2, 1, 1, 3), dtype=np.uint8)
                )
        ...
        # ----
        bbs_norm = normalization.normalize_bounding_boxes(
            ia.BoundingBox(x1=1, y1=2, x2=3, y2=4), shapes=[np.zeros((1, 1, 3), dtype=np.uint8)]
        )
        assert isinstance(bbs_norm, list)
        assert isinstance(bbs_norm[0], ia.BoundingBoxesOnImage)
        assert len(bbs_norm[0].bounding_boxes) == 1
        ...
        for dt in [np.dtype("float32"), np.dtype("int16"), np.dtype("uint16")]:
            bbs_norm = normalization.normalize_bounding_boxes(
                [np.zeros((1, 4), dtype=dt) + 1], shapes=[np.zeros((1, 1, 3), dtype=np.uint8)]
            )
            assert isinstance(bbs_norm, list)
            assert isinstance(bbs_norm[0], ia.BoundingBoxesOnImage)
            assert len(bbs_norm[0].bounding_boxes) == 1
            ...
            bbs_norm = normalization.normalize_bounding_boxes(
                [np.zeros((5, 4), dtype=dt) + 1], shapes=np.zeros((1, 1, 1, 3), dtype=np.uint8)
            )
            assert isinstance(bbs_norm, list)
            assert isinstance(bbs_norm[0], ia.BoundingBoxesOnImage)
            assert len(bbs_norm[0].bounding_boxes) == 5
            ...
            with self.assertRaises(ValueError):
                _bbs_norm = normalization.normalize_bounding_boxes(
                    [np.zeros((1, 4), dtype=dt) + 1], shapes=np.zeros((2, 1, 1, 3), dtype=np.uint8)
                )
        ...
        with self.assertRaises(ValueError):
            _bbs_norm = normalization.normalize_bounding_boxes(
                [[(1, 2, 3, 4), (3, 4, 5, 6)], [(5, 6, 7, 8), (7, 8, 9, 10)]], shapes=None
            )
    ...
    def test_normalize_polygons(self):
        ...
        for dt in [np.dtype("float32"), np.dtype("int16"), np.dtype("uint16")]:
            ...
            polygons_norm = normalization.normalize_polygons(
                np.tile(coords1_arr[np.newaxis, np.newaxis, ...].astype(dt), (1, 5, 1, 1)),
                shapes=np.zeros((1, 1, 1, 3), dtype=np.uint8),
            )
            assert isinstance(polygons_norm, list)
            assert isinstance(polygons_norm[0], ia.PolygonsOnImage)
            ...
            with self.assertRaises(ValueError):
                _polygons_norm = normalization.normalize_polygons(
                    np.tile(coords1_arr[np.newaxis, np.newaxis, ...].astype(dt), (2, 1, 1, 1)),
                    shapes=[np.zeros((1, 1, 3), dtype=np.uint8)],
                )
            ...
            with self.assertRaises(ValueError):
                _polygons_norm = normalization.normalize_polygons(
                    np.tile(coords1_arr[np.newaxis, np.newaxis, ...].astype(dt), (1, 1, 1, 1)),
                    shapes=np.zeros((2, 1, 1, 3), dtype=np.uint8),
                )
            ...
            with self.assertRaises(ValueError):
                _polygons_norm = normalization.normalize_polygons(
                    np.tile(coords1_arr[np.newaxis, np.newaxis, ...].astype(dt), (1, 1, 1, 10)),
                    shapes=np.zeros((1, 1, 1, 3), dtype=np.uint8),
                )

            _assert_single_image_expected(coords1_arr[np.newaxis, np.newaxis, ...].astype(dt))
        ...
        # ----
        polygons_norm = normalization.normalize_polygons(
            [ia.Polygon(coords1), ia.Polygon(coords2)], shapes=[np.zeros((1, 1, 3), dtype=np.uint8)]
        )
        assert isinstance(polygons_norm, list)
        assert isinstance(polygons_norm[0], ia.PolygonsOnImage)
        ...
        for dt in [np.dtype("float32"), np.dtype("int16"), np.dtype("uint16")]:
            ...
            with self.assertRaises(ValueError):
                _polygons_norm = normalization.normalize_polygons(
                    [[coords1_arr.astype(dt)]], shapes=np.zeros((2, 1, 1, 3), dtype=np.uint8)
                )
        ...
        # --> images None
        with self.assertRaises(ValueError):
            _polygons_norm = normalization.normalize_polygons([coords1, coords2], shapes=None)
    ...
    def test_normalize_line_strings(self):
        ...
        # ----
        lss_norm = normalization.normalize_line_strings(
            ia.LineStringsOnImage([ia.LineString(coords1)], shape=(1, 1, 3)), shapes=None
        )
        assert isinstance(lss_norm, list)
        assert isinstance(lss_norm[0], ia.LineStringsOnImage)
        assert len(lss_norm[0].line_strings) == 1
    ...
    def test__find_first_nonempty(self):
        ...
        # list of array
        observed = normalization.find_first_nonempty([np.zeros((4, 4, 3)), np.zeros((5, 5, 3))])
        assert ia.is_np_array(observed[0])
        assert observed[0].shape == (4, 4, 3)
        assert observed[1] is True
        assert len(observed[2]) == 1
        ...
        observed = normalization.find_first_nonempty(
            [(np.zeros((4, 4, 3)), np.zeros((5, 5, 3))), (np.zeros((6, 6, 3)), np.zeros((7, 7, 3)))]
        )
        assert ia.is_np_array(observed[0])
        assert observed[0].shape == (4, 4, 3)
        assert observed[1] is True
    ...
    def test__nonempty_info_to_type_str(self):
        ...
        with self.assertRaises(AssertionError):
            ntype = normalization._nonempty_info_to_type_str(1, True, [tuple([1, 2, 3])])
            assert ntype == "tuple[number,size=4]"
        ...
        ntype = normalization._nonempty_info_to_type_str(np.zeros((4, 4, 3), dtype=bool), True, [])
    ...
    def test_estimate_heatmaps_norm_type(self):
        ...
        ntype = normalization.estimate_heatmaps_norm_type(np.zeros((1, 1, 1, 1), dtype=np.float32))
        assert ntype == "array[float]"
        ...
        ntype = normalization.estimate_heatmaps_norm_type([np.zeros((1, 1, 1), dtype=np.float32)])
        ...
        with self.assertRaises(AssertionError):
            _ntype = normalization.estimate_heatmaps_norm_type(np.zeros((1, 1, 1), dtype=np.int32))
    ...
    def test_estimate_segmaps_norm_type(self):
        ...
        ntype = normalization.estimate_segmaps_norm_type(
            ia.SegmentationMapsOnImage(np.zeros((1, 1, 1), dtype=np.int32), shape=(1, 1, 1))
        )
        ...
        ntype = normalization.estimate_segmaps_norm_type([np.zeros((1, 1, 1), dtype=np.int32)])
    ...
    def test_estimate_keypoints_norm_type(self):
        ...
        with self.assertRaises(AssertionError):
            _ntype = normalization.estimate_keypoints_norm_type([[[ia.Keypoint(x=1, y=2)]]])
    ...
    def test_estimate_line_strings_norm_type(self):
        ...
        with self.assertRaises(AssertionError):
            _ntype = normalization.estimate_line_strings_norm_type([[[ia.LineString(points)]]])
