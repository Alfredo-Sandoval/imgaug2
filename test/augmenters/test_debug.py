...
class Test_draw_debug_image(unittest.TestCase):
    ...
    def _find_in_image_avg_diff(cls, find_image, in_image):
        ...
        top_left = min_loc
        bottom_right = (top_left[0] + find_image.shape[1], top_left[1] + find_image.shape[0])
        image_found = in_image[top_left[1] : bottom_right[1], top_left[0] : bottom_right[0], :]
    ...
    def test_two_images_and_polygons(self):
        ...
        for x in np.linspace(0, 256, 4):
            for y in np.linspace(0, 256, 4):
                polys.append(ia.Polygon([(x, y), (x + 20, y), (x + 20, y + 20), (x, y + 20)]))
    ...
    def test_two_images_and_line_strings(self):
        ...
        for x in np.linspace(0, 256, 4):
            for y in np.linspace(0, 256, 4):
                ls.append(ia.LineString([(x, y), (x + 20, y), (x + 20, y + 20), (x, y + 20)]))
