    ...
    def test_by_comparison_with_pil(self):
        ...
        for cutoff in cutoffs:
            for ignore in ignores:
                for i, image in enumerate(images):
                    with self.subTest(
                        cutoff=cutoff, ignore=ignore, image_idx=i, image_shape=image.shape
                    ):
                        ...
                        result_iaa = iaa.pillike.autocontrast(image, cutoff=cutoff, ignore=ignore)
        ...
        for nb_channels in nb_channels_lst:
            for size in [20]:
                for cutoff in [0, 1, 10]:
                    with self.subTest(nb_channels=nb_channels, size=size, cutoff=cutoff):
                        shape = (size, size, nb_channels)
                        image = iarandom.RNG(0).integers(50, 150, size=shape)
                        image = image.astype(np.uint8)
            ...
            for cutoff in [0, 1, 10]:
                for ignore in [None, 0, 1, [0, 1, 10]]:
                    with self.subTest(shape=shape, cutoff=cutoff, ignore=ignore):
                        ...
                        image_aug = iaa.pillike.autocontrast(image, cutoff=cutoff, ignore=ignore)
            ...
            for shape in shapes:
                for factor in factors:
                    with self.subTest(shape=shape, seed=seed, factor=factor):
                        image = iarandom.RNG(seed).integers(0, 256, size=shape, dtype="uint8")

                        image_iaa = func(image, factor)
                        image_pil = np.asarray(cls(PIL.Image.fromarray(image)).enhance(factor))
...
class Test_enhance_color(_TestEnhanceFunc):
    def test_by_comparison_with_pil(self):
        self._test_by_comparison_with_pil(iaa.pillike.enhance_color, PIL.ImageEnhance.Color)
...
class Test_enhance_contrast(_TestEnhanceFunc):
    def test_by_comparison_with_pil(self):
        self._test_by_comparison_with_pil(iaa.pillike.enhance_contrast, PIL.ImageEnhance.Contrast)
...
class Test_enhance_sharpness(_TestEnhanceFunc):
    def test_by_comparison_with_pil(self):
        self._test_by_comparison_with_pil(iaa.pillike.enhance_sharpness, PIL.ImageEnhance.Sharpness)
        ...
        for seed in seeds:
            for shape in shapes:
                with self.subTest(shape=shape, seed=seed):
                    ...
                    image_pil = np.asarray(PIL.Image.fromarray(image).filter(pil_kernel))
...
class Test_filter_smooth(_TestFilterFunc):
    def test_by_comparison_with_pil(self):
        self._test_by_comparison_with_pil(iaa.pillike.filter_smooth, PIL.ImageFilter.SMOOTH)
...
class Test_filter_find_edges(_TestFilterFunc):
    def test_by_comparison_with_pil(self):
        self._test_by_comparison_with_pil(iaa.pillike.filter_find_edges, PIL.ImageFilter.FIND_EDGES)
...
class Test_filter_contour(_TestFilterFunc):
    def test_by_comparison_with_pil(self):
        self._test_by_comparison_with_pil(iaa.pillike.filter_contour, PIL.ImageFilter.CONTOUR)
...
class Test_filter_emboss(_TestFilterFunc):
    def test_by_comparison_with_pil(self):
        self._test_by_comparison_with_pil(iaa.pillike.filter_emboss, PIL.ImageFilter.EMBOSS)
...
class Test_filter_sharpen(_TestFilterFunc):
    def test_by_comparison_with_pil(self):
        self._test_by_comparison_with_pil(iaa.pillike.filter_sharpen, PIL.ImageFilter.SHARPEN)
...
class Test_filter_detail(_TestFilterFunc):
    def test_by_comparison_with_pil(self):
        self._test_by_comparison_with_pil(iaa.pillike.filter_detail, PIL.ImageFilter.DETAIL)
...
class Test_warp_affine(unittest.TestCase):
            ...
            for seed in seeds:
                for fillcolor in fillcolors:
                    for arg_value in arg_values:
                        with self.subTest(
                            shape=shape, seed=seed, fillcolor=fillcolor, **{arg_name: arg_value}
                        ):
                            image = iarandom.RNG(seed).integers(0, 256, size=shape, dtype="uint8")
    ...
    def test_rotate_by_comparison_with_pil(self):
        def _matrix_gen(rotate):
            r = np.deg2rad(rotate)
            return np.float32([[np.cos(r), np.sin(r), 0], [-np.sin(r), np.cos(r), 0], [0, 0, 1]])
    ...
    def test_fillcolor_is_tuple(self):
        ...
        image_aug = iaa.pillike.warp_affine(image, translate_x_px=1, fillcolor=(2, 3, 4))
    ...
    def test_fillcolor_is_tuple_more_values_than_channels(self):
        ...
        image_aug = iaa.pillike.warp_affine(image, translate_x_px=1, fillcolor=(2, 3, 4, 5))
...
class TestEnhanceContrast(unittest.TestCase):
    ...
    def test_simple_image(self):
        ...
        diff_after = np.average(np.abs(image_aug.astype(np.int32) - np.average(image_aug)))
...
class FilterSmoothMore(_TestFilter):
    def test___init__(self):
        self._test___init__(iaa.pillike.FilterSmoothMore, iaa.pillike.filter_smooth_more)
...
class FilterEdgeEnhance(_TestFilter):
    def test___init__(self):
        self._test___init__(iaa.pillike.FilterEdgeEnhance, iaa.pillike.filter_edge_enhance)
...
class FilterEdgeEnhanceMore(_TestFilter):
    def test___init__(self):
        self._test___init__(iaa.pillike.FilterEdgeEnhanceMore, iaa.pillike.filter_edge_enhance_more)

    def test_image(self):
        self._test_image(iaa.pillike.FilterEdgeEnhanceMore, PIL.ImageFilter.EDGE_ENHANCE_MORE)
