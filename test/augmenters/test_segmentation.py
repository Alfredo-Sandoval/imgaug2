...
class TestSuperpixels(unittest.TestCase):
    ...
    @property
    def base_img(self):
        base_img = [[255, 255, 255, 0, 0, 0], [255, 235, 255, 0, 20, 0], [250, 250, 250, 5, 5, 5]]
        ...
        for use_np_replace in _NP_REPLACE:
            with self.subTest(use_np_replace=use_np_replace):
                with _create_replace_np_context(use_np_replace):
                    ...
                    observed = aug.augment_image(self.base_img)
                    assert np.allclose(observed, self.base_img) or self._array_equals_tolerant(
...
class TestVoronoi(unittest.TestCase):
    ...
    def test_determinism_integrationtest(self):
        ...
        image[:, :, 1] += 5
        image[:, :, 2] += 10
        sampler = iaa.DropoutPointsSampler(iaa.RegularGridPointsSampler((1, 10), (1, 20)), 0.5)
        ...
        same_between_a1_a2 = _array_lists_elementwise_identical(images_aug_a1, images_aug_a2)
        same_between_b1_b2 = _array_lists_elementwise_identical(images_aug_b1, images_aug_b2)
    ...
    def test___init___(self):
        ...
        with mock.patch(fname, mock_voronoi):
            _ = iaa.UniformVoronoi(
                100, p_replace=0.5, max_size=5, interpolation="cubic", seed=rs, name="foo"
            )

        assert mock_voronoi.call_count == 1
        assert isinstance(
            mock_voronoi.call_args_list[0][1]["points_sampler"], iaa.UniformPointsSampler
        )
...
class TestRegularGridVoronoi(unittest.TestCase):
    ...
    def test___init___integrationtest(self):
        ...
        aug = iaa.RegularGridVoronoi(
            10, (10, 30), p_replace=0.5, max_size=5, interpolation="cubic", seed=rs, name=None
        )
...
class TestRelativeRegularGridVoronoi(unittest.TestCase):
    ...
    def test___init___integrationtest(self):
        ...
        aug = iaa.RelativeRegularGridVoronoi(
            0.1, (0.1, 0.3), p_replace=0.5, max_size=5, interpolation="cubic", seed=rs, name=None
        )
    ...
    def test_pickleable(self):
        aug = iaa.RelativeRegularGridVoronoi((0.01, 0.2), (0.01, 0.2), p_replace=0.5, seed=1)
        runtest_pickleable_uint8_img(aug, iterations=3, shape=(50, 50, 1))
...
class TestRegularGridPointsSampler(unittest.TestCase):
    ...
    def test_conversion_to_string(self):
        ...
        expected = f"RegularGridPointsSampler({str(sampler.n_rows)}, {str(sampler.n_cols)})"
...
class TestUniformPointsSampler(unittest.TestCase):
    ...
    def test_sampled_points_uniformly_distributed_by_quadrants(self):
        ...
        np.add.at(n_points_per_quadrant, (points_quadrants[:, 1], points_quadrants[:, 0]), 1)
    ...
    def test_many_images(self):
        ...
        images = [np.zeros((100, 500, 3), dtype=np.uint8), np.zeros((500, 100, 1), dtype=np.uint8)]
    ...
    def test_n_points_can_vary_between_images(self):
        sampler = iaa.UniformPointsSampler(iap.Choice([1, 10]))
        images = [np.zeros((10, 10, 1), dtype=np.uint8) for _ in range(50)]
