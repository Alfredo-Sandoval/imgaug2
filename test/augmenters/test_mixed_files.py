...
class TestKeypointAugmentation(unittest.TestCase):
    ...
    def test_many_augmenters(self):
        ...
        augs = [
            ...
            iaa.BlendAlphaElementwise((0.0, 0.1), iaa.Add(10), name="BlendAlphaElementwise"),
            iaa.BlendAlphaSimplexNoise(iaa.Add(10), name="BlendAlphaSimplexNoise"),
            iaa.BlendAlphaFrequencyNoise(
                exponent=(-2, 2), foreground=iaa.Add(10), name="BlendAlphaSimplexNoise"
            ),
        ...
        for aug in augs:
            ...
            for i in range(10):
                ...
                kp_image_aug_rev = ia.KeypointsOnImage.from_keypoint_image(
                    kp_image_aug, if_not_found_coords={"x": -9999, "y": -9999}, nb_channels=1
                )
                ...
                for kp_pred, kp_pred_img in gen:
                    ...
                    if not kp_pred_lost and not kp_pred_img_lost:
                        d = np.sqrt(
                            (kp_pred.x - kp_pred_img.x) ** 2 + (kp_pred.y - kp_pred_img.y) ** 2
                        )
                        ds.append(d)
                dss.extend(ds)
                if len(ds) == 0:
                    print(
                        f"[INFO] No valid keypoints found for '{str(aug)}' "
...
def test_dtype_preservation():
    ...
    augs = [
        ...
        (iaa.Sequential([iaa.Identity(), iaa.Identity()], name="SequentialNoop"), default_dtypes),
        (iaa.SomeOf(1, [iaa.Identity(), iaa.Identity()], name="SomeOfNoop"), default_dtypes),
        (iaa.OneOf([iaa.Identity(), iaa.Identity()], name="OneOfNoop"), default_dtypes),
        (iaa.Sometimes(0.5, iaa.Identity(), name="SometimesNoop"), default_dtypes),
        (
            iaa.Sequential([iaa.Add((-5, 5)), iaa.AddElementwise((-5, 5))], name="Sequential"),
            _not_dts([np.uint32, np.int32, np.float64]),
        ),
        (
            iaa.SomeOf(1, [iaa.Add((-5, 5)), iaa.AddElementwise((-5, 5))], name="SomeOf"),
            _not_dts([np.uint32, np.int32, np.float64]),
        ),
        (
            iaa.OneOf([iaa.Add((-5, 5)), iaa.AddElementwise((-5, 5))], name="OneOf"),
            _not_dts([np.uint32, np.int32, np.float64]),
        ),
        (
            iaa.Sometimes(0.5, iaa.Add((-5, 5)), name="Sometimes"),
        ...
        (
            ...
            _not_dts([np.float64]),
        ),  # float64 requires float128 support
        (
            iaa.BlendAlphaElementwise(
                (0.0, 0.1), iaa.Identity(), name="BlendAlphaElementwiseIdentity"
            ),
            _not_dts([np.float64]),
        ),  # float64 requires float128 support
        (
            iaa.BlendAlphaSimplexNoise(iaa.Identity(), name="BlendAlphaSimplexNoiseIdentity"),
            _not_dts([np.float64]),
        ),  # float64 requires float128 support
        (
            iaa.BlendAlphaFrequencyNoise(
                exponent=(-2, 2), foreground=iaa.Identity(), name="BlendAlphaFrequencyNoiseIdentity"
            ),
            _not_dts([np.float64]),
        ),
        (
        ...
        (
            iaa.BlendAlphaFrequencyNoise(
                exponent=(-2, 2), foreground=iaa.Add(10), name="BlendAlphaFrequencyNoise"
            ),
            _not_dts([np.uint32, np.int32, np.float64]),
        ),
        (
        ...
        (
            iaa.Resize({"height": 4, "width": 4}, name="Resize"),
            _not_dts(
                [np.uint16, np.uint32, np.int16, np.int32, np.float32, np.float16, np.float64]
            ),
        ),
        (
            iaa.CropAndPad(px=(-10, 10), name="CropAndPad"),
